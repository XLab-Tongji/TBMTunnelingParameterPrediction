...
let CompareData=[
    //元数据
    [15713.91974, 16015.02828, 15891.69147, 16218.16112, 16051.47614, 18342.42524, 17358.14431, 12261.25375, 16975.58267, 17423.11741,],
    [2942.392641, 3013.518114, 3179.002699, 3119.361978, 3172.595804, 3013.220918, 2657.163266, 1370.768179, 3200.057552, 3066.926265,],
    [60.83314267,
        67.05818797,
        73.1904141,
        69.30692463,
    ...
    data() {
        return {
            ...
            option2:{
                ...
                legend: {
                    data: ['原数据', 'TNN预测结果','比对模型1预测结果','比对模型2预测结果']
                },
                grid: {
                    left: '3%',
                    right: '4%',
    ...
    methods:{
        ...
        TestResult(command){
            if(command=="a")
            {
                ...
                this.$http.post(this.baseURL+'api/TNN模型测试数据1',{temp})
                    .then(function(res){
            ...
            {
                let temp=JSON.stringify({"预测数据":this.TableData});
                this.$http.post(this.baseURL+'api/TNN模型测试数据2',{temp})
                    .then(function(res){
                        this.Result=undefined;
                        this.Result=res.data.filter(l=>l.name="index");
                        CompareData[3]=res.data.filter(l=>l.name="result1")[0];
