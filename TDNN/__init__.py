...
def data_preparation(model, data_csv, target='F'):
    ...
    cols = df_X.columns.values.tolist()
    print(cols)
    NN_num = model.leaf_NN_num
    input_num_for_each_NN = model.leaf_input_num
    # NN_num = model['leaf_NN_num']
    # input_num_for_each_NN = model['leaf_input_num']

    for i in range(NN_num):
        cur_index = i * input_num_for_each_NN
        cur_train_cols = cols[cur_index: cur_index + input_num_for_each_NN]
        if i == NN_num-1:
            cur_train_cols = cols[cur_index:]
        train_leaf_X_list.append(np.array(train_X[cur_train_cols]))
        test_leaf_X_list.append(np.array(test_X[cur_train_cols]))
